...
interface InterviewSessionProps {
  ...
  onSubmitAnswer: (answer: string, codeLanguage?: string, codeOutput?: string) => Promise<void>;
...
export function getInterviewerProfile(personality: string, company?: string, actuarialFocus?: string): InterviewerProfile {
...
export default function InterviewSessionComponent({
  ...
  const interviewer = getInterviewerProfile(session.personality, session.company, session.actuarialFocus);
  ...
  const [backgroundPreset, setBackgroundPreset] = useState<"boardroom" | "techlab" | "office">("boardroom");
  ...
  }, [session.currentQuestionIndex, currentQuestion, codeLanguage]);
  ...
  const stopVoiceRecording = () => {
    ...
    setTimeout(() => {
      if (!typedAnswer.trim()) {
        ...
        setTimeout(() => {
          setTypedAnswer(getSimulatedSpeech(currentQuestion?.type));
          setIsTranscribing(false);
        }, 1400);
      }
    }, 100);
  ...
  // Build the boardroom backdrop layout styling
  const getBackdropClass = () => {
    switch(backgroundPreset) {
      case "techlab":
        return "bg-slate-950 border-indigo-500/30 shadow-[inset_0_0_50px_rgba(99,102,241,0.15)]";
      case "office":
        return "bg-zinc-900 border-amber-500/10 shadow-[inset_0_0_50px_rgba(245,158,11,0.05)]";
      default: // boardroom
        return "bg-slate-900 border-slate-800 shadow-2xl shadow-indigo-950/25";
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch" id="immersive-interview-workspace">
      ...
      <div className={`lg:col-span-7 rounded-2xl border p-5 flex flex-col justify-between text-white transition-all duration-300 relative overflow-hidden ${getBackdropClass()}`}>
        ...
        <div className="my-6 grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1 items-stretch z-10">
          ...
          <div className="bg-black/40 rounded-xl border border-white/5 p-4 flex flex-col justify-between items-center relative overflow-hidden group">
            ...
            <div className="flex-1 flex flex-col items-center justify-center space-y-3.5 py-4">
              <div className={`w-20 h-20 rounded-full bg-slate-800 border-2 flex items-center justify-center text-4xl shadow-lg transition-all duration-300 ${
                isSpeaking ? "border-indigo-400 scale-105 shadow-indigo-500/25 ring-4 ring-indigo-500/10" : "border-slate-700"
              }`}>
                {interviewer.avatar}
              </div>
        ...
        <div className="bg-black/30 backdrop-blur-md border border-white/5 rounded-xl p-4 text-left z-10 space-y-2">
          <div className="flex justify-between items-start">
            ...
            >
              ...
              <span>{isSpeaking ? "Mute Coach" : "Read Question"}</span>
            </button>
          </div>
        ...
        <div className="flex flex-wrap justify-between items-center bg-black/30 backdrop-blur-md px-4 py-3 rounded-xl border border-white/5 mt-5 z-10">
          {/* Left panel: Background Preset switch */}
          <div className="flex items-center gap-1">
            <button 
              onClick={() => setBackgroundPreset("boardroom")}
              className={`p-1.5 rounded text-[9px] font-bold uppercase transition ${
                backgroundPreset === "boardroom" ? "bg-indigo-500 text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
              }`}
              title="Boardroom Backdrop"
            >
              <Monitor size={12} className="inline mr-1" /> Boardroom
            </button>
            <button 
              onClick={() => setBackgroundPreset("techlab")}
              className={`p-1.5 rounded text-[9px] font-bold uppercase transition ${
                backgroundPreset === "techlab" ? "bg-indigo-500 text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
              }`}
              title="Tech Lab Backdrop"
            >
              <Code size={12} className="inline mr-1" /> Lab
            </button>
            <button 
              onClick={() => setBackgroundPreset("office")}
              className={`p-1.5 rounded text-[9px] font-bold uppercase transition ${
                backgroundPreset === "office" ? "bg-indigo-500 text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
              }`}
              title="Modern Office Backdrop"
            >
              <Image size={12} className="inline mr-1" /> Office
            </button>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className={`p-2.5 rounded-full transition ${
                micActive ? "bg-slate-800 text-slate-300 hover:bg-slate-700" : "bg-rose-500 text-white"
              }`}
            ...
            <button 
              ...
              className={`p-2.5 rounded-full transition ${
                cameraActive ? "bg-slate-800 text-slate-300 hover:bg-slate-700" : "bg-rose-500 text-white"
              }`}
              title={cameraActive ? "Camera OFF" : "Camera ON"}
            >
      ...
      <div className="lg:col-span-5 bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col justify-between" id="candidate-workspace">
        ...
        {currentQuestion.type === "coding" ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center space-y-6 py-6 text-center">
            ...
            {/* Audio reviewer player */}
            {recordedAudioUrl && !isRecording && (
              <div className="w-full max-w-xs bg-slate-50 border border-slate-200 p-3 rounded-xl space-y-1.5">
                <div className="flex justify-between items-center text-[9px]">
                  <span className="font-bold text-slate-500 uppercase">Review Recorded Audio</span>
                  <button 
                    onClick={() => {
                      setRecordedAudioUrl(null);
                      setTypedAnswer("");
